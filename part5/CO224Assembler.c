...
Modified by  :  Imesh Balasuriya (E/17/018), Achintha Harshamal (E/17/090)

*Added support for bne, mult, sll, srl, sra and ror instructions*
	...
	*************************************************************************/
 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*************************************************************************/
		...
		if(strcasecmp(in_token,"mov")==0 || strcasecmp(in_token,"loadi")==0 || strcasecmp(in_token,"j")==0) 
		{
		...
		else if(strcasecmp(in_token,"sll")==0 || strcasecmp(in_token,"srl")==0 || strcasecmp(in_token,"sra")==0 || strcasecmp(in_token,"ror")==0)
		{
			...
			int shift = (strcasecmp(in_token,"sll")==0 || strcasecmp(in_token,"srl")==0 || strcasecmp(in_token,"sra"))?1:0; //Flag if this is a shift instruction
			int arith = (strcasecmp(in_token,"sra")==0)?1:0; //Flag if this is a 'sra' instruction
			int rotate = (strcasecmp(in_token, "ror")==0)?1:0;	//Flag if rotate
			...
			in_token = strtok(NULL, delim); // Read the third token (RT)
			strcat(pline,in_token); strcat(pline,delim);// Write RT to the pre-processed line, with a delimiter
			
			
			in_token = strtok(NULL, delim); // Read the fourth token (RS/IMM)
			
			if (strstr(in_token,"0x") && (strstr(in_token,"0x") == in_token)){
				
				strcpy(shiftAmount,in_token);	//Copy the fourth token to shiftAmount
				
				if (shift){
					//If shift amount is greater than 8 set it to 8
					int i;
					for(i=0;i<2;i++)
					{
						if ((toupper(in_token[2+i])=='8') || (toupper(in_token[2+i])=='9') || (toupper(in_token[2+i])=='A') || (toupper(in_token[2+i])=='B') || 
						(toupper(in_token[2+i])=='C') || (toupper(in_token[2+i])=='D') || (toupper(in_token[2+i])=='E') || (toupper(in_token[2+i])=='F')) {
							strcpy(shiftAmount,"0x08");
							break;
						}
					}
				}
				
				if (arith) {	//Alter shift value for arithmetic shift
					//Set shift amount to 0x4n
					shiftAmount[2] = '4';
				}
				else if (rotate) {		//Alter shift value for rotate
					//Set shift amount to 0x8n
					shiftAmount[2] = '8';
				}
				
				strcat(pline,shiftAmount); strcat(pline,delim);// Write Shift Amount to the pre-processed line, with a delimiter
			}
			while(1)
			{
				in_token = strtok(NULL, delim);// Read any remaining token (comments etc.)
			...
			// The line is either not an instruction, or an instruction with all four tokens and not a shift instruction
			strcpy(pline,line);
