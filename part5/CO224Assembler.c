...
Author	: Isuru Nawinne (Extended for MULT instruction)
Date		: 14-April-2020
...
This simple assembler assumes an ISA containing the following instructions: loadi, mov, add, sub, and, or, j, beq, lwd, lwi, swd, swi, mult, sll, srl, sra, ror, bne. 
All instructions are encoded into 32-bit words based on the following format:

Bits 31-24 : OP-CODE				: Given as one of (loadi, mov, add, sub, and, or, j, beq, lwd, lwi, swd, swi, mult, sll, srl, sra, ror, bne)
...
This assembler will perform some basic error checks on your program. A valid instruction should contain two to four tokens separated by space character (e.g. "add 3 2 1", "mult 5 3 2", "loadi 5 0x1A", "j 0x0A"), corresponding to the details given above. In addition, empty lines and comments are permitted. A valid comment should start with "//".
